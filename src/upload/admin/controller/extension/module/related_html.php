...
class ControllerExtensionModuleRelatedHTML extends Controller {
	...
	*/
	...
	private $_entity_data = [
		...
		'manufacturer' => [
			...
			'query_selector'	=> '#form-manufacturer'
		]
	];
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validatePermissionAndName()) {
			...
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('related_html', $this->request->post);
		...
		// top menu
		if (!isset($this->request->get['module_id'])) {
			$data['save'] = $this->url->link($this->_route, 'token=' . $this->session->data['token'], true);
		} else {
			$data['save'] = $this->url->link($this->_route, 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		$this->load->model('extension/module');
		$data['modules'] = $this->model_extension_module->getModulesByCode('related_html');
		...
		$data['list'] = $this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'], true);
		$data['add_block'] = $this->url->link($this->_route . '/add', 'token=' . $this->session->data['token'], true);
		$data['htmls_top'] = $this->{$this->_model}->getAllRelatedHTMLs(false);
		foreach ($data['htmls_top'] as $key => $value) {
			$data['htmls_top'][$key]['edit'] = $this->url->link($this->_route . '/edit', 'token=' . $this->session->data['token'] . '&html_id=' . $value['html_id'], true);
		}
	...
	*/
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validatePermissionAndName()) {
			...
			$this->response->redirect($this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'] . $url . '&html_id=' . (int) $html_id, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validatePermissionAndName()) {
			...
			$this->response->redirect($this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'] . $url . '&html_id=' . (int) $this->request->get['html_id'], true));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validatePermission()) {
			...
			$this->response->redirect($this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = [
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'] . $url, true)
		];

		// top menu
		$data['add_block'] = $this->url->link($this->_route . '/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['clone'] = $this->url->link($this->_route . '/clone', 'token=' . $this->session->data['token'] . $url, true);
		$data['multiple_edit'] = $this->url->link($this->_route . '/multipleEdit', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link($this->_route . '/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['list'] = $this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'] . $url, true);
		$data['add_block'] = $this->url->link($this->_route . '/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['htmls_top'] = $this->{$this->_model}->getAllRelatedHTMLs(false);
		foreach ($data['htmls_top'] as $key => $value) {
			$data['htmls_top'][$key]['edit'] = $this->url->link($this->_route . '/edit', 'token=' . $this->session->data['token'] . '&html_id=' . $value['html_id'] . $url, true);
		}
		...
		foreach ($htmls as $key => $html_info) {
			$data['htmls'][] = [
				...
				'edit' => $this->url->link($this->_route . '/edit', 'token=' . $this->session->data['token'] . '&html_id=' . $html_info['html_id'] . $url, true)
			];
		}
		...
		$data['sort_name'] = $this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'] . '&sort=name' . $url , true);
		$data['sort_sort_order'] = $this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url , true);
		$data['sort_date_added'] = $this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url , true);
		$data['sort_date_edited'] = $this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'] . '&sort=date_edited' . $url , true);

		$data['change_status'] = html_entity_decode($this->url->link($this->_route . '/changeStatus', 'token=' . $this->session->data['token'], true), ENT_QUOTES, 'UTF-8');
		...
		$pagination->url = $this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($html_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($html_total - $this->config->get('config_limit_admin'))) ? $html_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $html_total, ceil($html_total / $this->config->get('config_limit_admin')));
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['html_id'])) {
			$data['save'] = $this->url->link($this->_route . '/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['save'] = $this->url->link($this->_route . '/edit', 'token=' . $this->session->data['token'] . '&html_id=' . (int) $this->request->get['html_id'] . $url, true);
		}
		$data['cancel'] = $this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'], true);
		...
		if (isset($this->request->post['name'])) {
			$data['name'] = $this->request->post['name'];
		} elseif (isset($html_info) && !empty($html_info)) {
	...
	public function multipleEdit() {
		...
		// top menu
		$data['save'] = $this->url->link($this->_route . '/multipleSave', 'token=' . $this->session->data['token'] . $url, true);
		$data['cancel'] = $this->url->link($this->_route . '/list', 'token=' . $this->session->data['token'], true);
	...
	*/
	...
	public function eventRHTMLFormViewAfter(&$route, &$data, &$output) {
		...
		$data['add'] = $this->url->link($this->_route . '/add', 'token=' . $this->session->data['token'], true);
