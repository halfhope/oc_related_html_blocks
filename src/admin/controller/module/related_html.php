...
namespace Opencart\Admin\Controller\Extension\RelatedHTML\Module;
class RelatedHTML extends \Opencart\System\Engine\Controller {
	
	private $_route 	= 'extension/related_html/module/related_html';
	private $_model 	= 'model_extension_related_html_module_related_html';
	...
	*/
	...
	public function index() {
		...
		// init
		$this->document->addScript('/extension/related_html/admin/view/javascript/jquery/switcher/jquery.switcher.min.js');
		$this->document->addStyle('/extension/related_html/admin/view/javascript/jquery/switcher/switcher.css');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validatePermissionAndName()) {
			...
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('related_html.related_html', $this->request->post);
		...
		$this->load->model('setting/module');
		$data['modules'] = $this->model_setting_module->getModulesByCode('related_html.related_html');
	...
	*/
	...
	protected function getList() {
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $limit,
			'url'   => $this->url->link($this->_route . '|list', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', 'SSL')
		]);
	...
	protected function getForm() {
		...
		$data['cancel'] = $this->url->link($this->_route . '|list', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
	...
	protected function validatePermissionAndName() {
		...
		if ((mb_strlen($this->request->post['name']) <= 0) || (mb_strlen($this->request->post['name']) > 255)) {
	...
	protected function validatePermissionAndMultipleName() {
		...
		if (isset($this->request->post['htmls'])) {
			foreach ($this->request->post['htmls'] as $html_id => $value) {
				if ((mb_strlen($value['name']) <= 0) || (mb_strlen($value['name']) > 255)) {
	...
	*/
	...
	public function eventRHTMLFormViewAfter(&$route, &$data, &$output) {
		...
		$path = explode('/', str_replace('|', '/', $this->request->get['route']));
		list($function, $controller) = [array_pop($path), array_pop($path)];

		$value = isset($this->request->get[$this->_entity_data[$controller]['column']]) ? $this->request->get[$this->_entity_data[$controller]['column']] : 0;
	...
	public function eventRHTMLFormModelAfter(&$route, &$args, &$output) {
		...
		$path = explode('/', str_replace('|', '/', $route));
		list($function, $controller) = [array_pop($path), array_pop($path)];

		// get category_id, product_id, manufacturer_id for addProduct/editProduct/deleteProduct
		$value = (strripos($function, 'add') === false) ? $args[0] : $output;

		// get related_htmls for addProduct/editProduct/deleteProduct
		$passed = !(strripos($function, 'edit') === false) ? $args[1] : $args[0];
